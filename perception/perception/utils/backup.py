...
class DepthDBSCANVisualizer:
    ...
    # ✅ self 추가 (여기가 에러 해결 포인트)
    def _scale_poly(self, poly: np.ndarray, scale: float) -> np.ndarray:
        """
        poly: (N,2) 정수/실수 상관없음
        scale: 1.0이면 동일, 1.2면 20% 확대
        """
        p = poly.astype(np.float32)
        c = p.mean(axis=0, keepdims=True)       # 중심
        p2 = (p - c) * float(scale) + c         # 중심 기준 확대
    ...
    def run(self):
        ...
        # ✅ vis에서만 확대해서 그리기 (초록/파랑 공통)
        for item in items:
            color = (0, 255, 0) if item["type"] == "green" else (255, 0, 0)

            box = item["poly"]  # 원본
            draw_box = self._scale_poly(box, self.VIS_SCALE)

            # 화면 밖 클램프 (권장)
            h, w = vis.shape[:2]
            draw_box[:, 0] = np.clip(draw_box[:, 0], 0, w - 1)
            draw_box[:, 1] = np.clip(draw_box[:, 1], 0, h - 1)

            cv2.drawContours(vis, [draw_box], 0, color, 2)

            # 중심은 원본 기준 (스케일해도 중심 동일)
            cx, cy = box_center_pixel(box)
            cv2.circle(vis, (cx, cy), 5, (0, 255, 0), -1)

            text_y = int(np.min(draw_box[:, 1]) - 10)
            cv2.putText(
                vis, f"{item['id']}",
                (cx, text_y),
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2

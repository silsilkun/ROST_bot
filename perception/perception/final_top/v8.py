...
# - Yaw computed from IMAGE PCA (stable) -> mapped to WORK yaw via (offset, sign)
# - Near-circular: yaw=None (viewer holds last_yaw), not forced 0.0
# - pipeline.start() busy guard + stop() only when started
...
def wrap_deg(a: float) -> float:
    ...
    # make 180 inclusive on positive side if you want; keep as is
    return float(x)
...
class PickConfig:
    ...
    # =====================================================
    # YAW_MODE:
    #   "image" : yaw from PCA in image plane (stable) then mapped to work via offset/sign
    #   (world P1/P2 방식은 좌표계/깊이 노이즈로 흔들려서 여기선 제거)
    YAW_MODE: str = "image"

    # image yaw = atan2(vy, vx) in degrees (x right, y down)
    # work yaw = wrap( YAW_SIGN * (yaw_img_deg - YAW_OFFSET_DEG) )
    #
    # Typical top-down camera:
    # - If object vertical (down direction) should become ~+90 or -90 depending on your robot,
    #   tune OFFSET/SIGN once.
...
def pca_axis_from_mask(mask_bool: np.ndarray, cfg: PickConfig):
    ...
    # ✅ Sign stabilization:
    # Force the axis to point "up" in image coordinates (y decreasing).
    # This removes random 180° flips across frames.
    if vy > 0.0:
        vx, vy = -vx, -vy

    yaw_img_deg = float(np.degrees(np.arctan2(vy, vx)))  # image coord: x right, y down
...
def compute_pick_from_snapshot(depth_snap_m: np.ndarray, cfg: PickConfig, coord: Coordinate) -> PickResult:
    ...
    if pca_info is not None:
        if pca_ratio < cfg.PCA_ANISO_RATIO_MIN:
            # ✅ policy change: near-circular => yaw is unreliable => None (viewer will hold last yaw)
            yaw_work_deg = None
            yaw_work_rad = None
            u2, v2 = None, None
        else:
            # image yaw -> work yaw mapping
            # work = wrap( sign * (img - offset) )
            yaw_work_deg = wrap_deg(cfg.YAW_SIGN * (yaw_img_deg - cfg.YAW_OFFSET_DEG))
            yaw_work_rad = float(np.radians(yaw_work_deg))

            # arrow (image direction)

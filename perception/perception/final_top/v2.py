...
# 4) pick point: dist 상위 90% 후보 중 centroid에 가장 가까운 점
...
TOP_PERCENT_PRIMARY = 3     # 1차 top percentile
TOP_PERCENT_FALLBACK = 5    # 2차(재시도) top percentile

# adaptive band clamp (meters)
BAND_MIN_M = 0.008   # 0.8 cm
BAND_MAX_M = 0.030   # 3.0 cm
BAND_MAD_K = 3.0     # band = K * MAD (robust)
...
# pick 후보 (dist 상위 비율)
DIST_TOP_RATIO = 0.90  # dist >= 0.90*dmax
...
def choose_pick_point_from_dist(dist: np.ndarray, mask_bool: np.ndarray):
    """
    A안: dist 상위 후보(>=0.90*dmax) 중 centroid에 가장 가까운 점 선택
    return: (u, v, dmax, chosen_dist)
    """
    dmax = float(dist.max())
    if dmax <= 0:
        return None
    ...
    # 후보 마스크
    cand = (dist >= (DIST_TOP_RATIO * dmax)) & mask_bool
    ys, xs = np.where(cand)

    # 후보가 너무 적으면 fallback: argmax
    if xs.size < 10:
        v0, u0 = np.unravel_index(int(dist.argmax()), dist.shape)
        return int(u0), int(v0), dmax, float(dist[v0, u0])

    # centroid (후보가 아닌 전체 mask centroid를 써도 됨. 여기선 cand centroid 사용)
    yc = float(ys.mean())
    xc = float(xs.mean())

    # centroid에 가장 가까운 후보점
    idx = int(np.argmin((xs - xc) ** 2 + (ys - yc) ** 2))
    u = int(xs[idx])
    v = int(ys[idx])
...
def compute_pick_snapshot(pipeline, align, roi_xyxy, coord: Coordinate):
    ...
    # 3) top mask primary / fallback (2-stage)
    ...
    # if mask is too small, retry with fallback percentile
    ...
    # 4) connected components -> best blob (score includes dist_max)
    ...
    # 5) distance transform on best -> pick point selection (top dist candidates)
    dist = cv2.distanceTransform(best.astype(np.uint8), cv2.DIST_L2, 5)
    pick = choose_pick_point_from_dist(dist, best)
    ...
    # 6) world conversion (single point)

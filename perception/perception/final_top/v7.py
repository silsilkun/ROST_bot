...
# All-in-one: RealSense viewer + snapshot median depth + pick + PCA yaw (robust)
# - Handles device busy (EBUSY) start error safely
# - pipeline.stop() only if started
# - Robust yaw: search P2 along PCA axis inside mask + multi-step + both directions
# - last_yaw hold: if yaw None this frame, reuse last valid yaw
# - Uses depth_snap_m (median) for BOTH segmentation and pixel->world Z
...
class PickConfig:
    ...
    # yaw robustness
    YAW_STEP_LIST_PX: Tuple[int, ...] = (12, 18, 25, 35, 50, 70)
    YAW_MIN_WORLD_DIST_CM: float = 0.6
...
def compute_world_yaw_from_pca_robust(
    ...
    for sx, sy in dirs:
        for step in cfg.YAW_STEP_LIST_PX:
            ...
            return {"yaw_work_rad": yaw, "yaw_work_deg": float(np.degrees(yaw)), "u2": u2, "v2": v2}
            ...
            if key == ord(" "):
                ...
                if res.ok and res.xyz_approach_cm is not None:
                    ...
                    if yaw_to_send is not None:
                        ...
                        print(f"SEND → xyz={list(xyz)} yaw_deg={yaw_to_send:.1f}{held_txt}")
                    else:

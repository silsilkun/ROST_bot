...
class PromptConfig:
    ...
    def get_prompt(self, expected_count: int) -> str:
        return (
            ...
            "\n"
            "Two images are provided from perception to estimation: a raw_image and a boxing_image. "
            "Use only the single numeric ID displayed directly above each bounding box in the boxing_image "
            "to identify and classify the corresponding objects in the raw_image, "
            "then output the object types in the exact order of their IDs."
            "\n"
            "### CRITICAL RULES:\n"
            "- A can viewed from the top is still 'can' if you see metallic sheen, a circular rim, or a pull-tab.\n"
            "- Do not confuse a can top with a paper carton cap; paper cartons have blue plastic caps, not metallic rims.\n"
            "- Yogurt/milk/juice cartons with printed paper surfaces are ALWAYS 'paper', even if glossy or reflective.\n"
            "\n"
            ...
            "### [2-C. Target-oriented Prompt Set for PAPER (Crumpled Paper, learn exactly from these)]"
